...
def wxmi(phones, psws, nums):  # 这个是执行刷步数的函数
    ...
    def main(user, passwd, step):
        ...
        result = f"用户：<span style='color:red'>{user[:5]}**{user[-4:]}</span>修改步数<span style='color:skyblue'>{step}</span>步 " + \
                 response['message'] + '<br>'
        # print(result)
        return result
...
def add_user():  # 这个是添加自动刷步数用户的函数
    ...
    def judge_phone(tel):
        ...
        sql = """select num from userlist where phone = """ + tel
        cursors = db.cursor()
        cursors.execute(sql)
        if ret is None:
            return "你输入的账号有误，请输入小米运动的账号（手机号）！"
        elif len(cursors.fetchall()) == 1:
            return "你已经在自动刷步数队列中，请勿重复添加！"
    ...
    if login_judge(str(phone), str(psw)) == 0:
        print("登陆失败,账号或密码错误")
        popup('错误提示', '添加失败，账号或密码错误！（请刷新网页）', size=PopupSize.SMALL)
        put_text('别看了，刷新就对了！').style("color:red")
    else:
        print(f"添加用户：{phone}成功")
        cursor = db.cursor()
        cursor.executemany(inesrt_data, userlist)
        db.commit()
        cursor.close()
...
if __name__ == '__main__':
    threads = threading.Thread(target=exe_push)
    threads.start()
    createdb()
    weipyweb.config(title="运动刷步数助手(自动版)", description="acloudtwei个人开发网站",
                    ...
                    , css_file=[""])
    start_server(add_user, port=80, host="0.0.0.0")
